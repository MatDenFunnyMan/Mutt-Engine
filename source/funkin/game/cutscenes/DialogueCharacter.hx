...
class DialogueCharacter extends FlxSprite
{
	...
	public function reloadCharacterJson(character:String) {
		...
		if (!FileSystem.exists(path)) {
			path = Paths.getRoutedSharedPath(characterPath);
		}

		if(!FileSystem.exists(path)) {
			path = Paths.getRoutedSharedPath('images/dialogue/' + DEFAULT_CHARACTER + '.json');
		}
		rawJson = File.getContent(path);

		#else
		var path:String = Paths.getRoutedSharedPath(characterPath);
