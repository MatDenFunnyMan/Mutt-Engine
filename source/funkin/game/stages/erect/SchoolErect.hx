...
class SchoolErect extends BaseStage
{
	...
	override function create()
	{
		...
		if (!seenCutscene && songName.endsWith('(pico-mix)'))
		{
			var cutscene = new funkin.game.cutscenes.SchoolDoof(songName);
			cutscene.onLine = onDialogueLine;
			if (songName == 'roses-(pico-mix)') setStartCallback(cutscene.doAngryIntro);
			else setStartCallback(cutscene.doSchoolIntro);
		}
	}

	var wasInit:Bool = true;
	function onDialogueLine(dialogueCount:Int)
	{
		if (songName != 'senpai-(pico-mix)') return;
	...
	override function createPost(){
		...
		if(ClientPrefs.data.shaders) {
			...
			if(pico != null && pico.abotPixel != null && game.stages.contains(pico))
			{
				applyShader(pico.abotPixel.speakerTop, "speakerTop");
				applyShader(pico.abotPixel.speaker, "");
				applyShader(pico.abotPixel.eyes, "");
			}
		}
